...
class Abst_symV: public Abst{
 ...
 public:
  ...
  void join(Abst_symV* a){
	...
	if(exp.size() == aexp.size()){
	  ...
	  for(it = exp.begin(); it !=exp.end(); it++){
	    ...
	    if(it2 != aexp.end()){
	      if(it2->second != it->second) flag = true;
	    }
	    else flag = true;
	  }
	}
    ...
    if(flag){
      if(_symv.contains(var(_last))) _symv = _symv.remove({var(_last)});
  ...
  void rename(var oldV, var newV){
    ...
    if(var(_last) == oldV) {std::stringstream l; l<<newV; _last = l.str();}
  }
  ...
  void assign(var dst, linexpr1 aexpr){
    ...
    for(bit = expr.begin(); bit != expr.end(); bit++)
      if(bit->second != 0) {flag = false; break;}
    if(flag){
